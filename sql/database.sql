...
-- 爬蟲練習表
CREATE TABLE IF NOT EXISTS crawler_challenges (
    id INT AUTO_INCREMENT PRIMARY KEY,
    location_id INT NOT NULL COMMENT '世界地圖位置ID',
    theme VARCHAR(100) NOT NULL COMMENT '爬蟲主題',
    difficulty VARCHAR(10) NOT NULL COMMENT '難度級別',
    html_content TEXT COMMENT '生成的HTML內容',
    hidden_answer VARCHAR(20) NOT NULL COMMENT '隱藏的答案',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 爬蟲練習紀錄表
CREATE TABLE IF NOT EXISTS crawler_challenge_logs (
    id INT AUTO_INCREMENT PRIMARY KEY,
    player_id INT NOT NULL,
    challenge_id INT NOT NULL,
    attempt_count INT DEFAULT 1 COMMENT '嘗試次數',
    is_solved BOOLEAN DEFAULT FALSE COMMENT '是否解決',
    time_spent INT COMMENT '花費時間(秒)',
    hint_used BOOLEAN DEFAULT FALSE COMMENT '是否使用提示',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    solved_at TIMESTAMP NULL,
    FOREIGN KEY (player_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (challenge_id) REFERENCES crawler_challenges(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 修改users表，加入尋寶次數欄位
ALTER TABLE users ADD COLUMN IF NOT EXISTS treasure_hunts INT DEFAULT 0 COMMENT '完成的尋寶次數';
...
-- 插入爬蟲世界地點主題
INSERT INTO crawler_challenges (location_id, theme, difficulty, hidden_answer) VALUES
(1, 'Python數據分析與資料處理', '初階', 'ans_data123'),
(2, 'Python字典與JSON處理', '中階', 'ans_dict456'),
(3, 'Python模組與套件', '中階', 'ans_mod789'),
(4, 'Python異常處理與除錯', '高階', 'ans_try000');
